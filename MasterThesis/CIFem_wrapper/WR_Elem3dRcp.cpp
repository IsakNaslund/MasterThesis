...
CIFem_wrapper::WR_Elem3dRcp::WR_Elem3dRcp(WR_XYZ stPos, WR_XYZ enPos, WR_ReleaseBeam3d stRel, WR_ReleaseBeam3d enRel, WR_IXSec xSec, double matStiff, double poisonRatio, std::vector<double> normal)
{
	_eleRcp = new Element3dRcp(stPos.GetPos(), enPos.GetPos(), stRel.GetRelease(), enRel.GetRelease(), xSec.GetCrossSection(), matStiff, poisonRatio, normal);
}
